...
def load_data(
    ...
    nonreg_trg_vocab = build_vocab(
        field="nonreg_trg",
        min_freq=nonreg_trg_min_freq,
...
class SignProdDataset(data.Dataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        src_path, trg_path, nonreg_trg_path, file_path = tuple(
        ...
        try:
            ...
            for file_paths, (_) in tar.items():
                ...
                examples.append(
                    data.Example.fromlist(
                        [src[:], trg[: num_sec * trg_fps], nonreg_trg_line, file_paths],
                        fields,
                    )
                )
                num_vids += 1
            ...
            ) as files_file, io.open(
                nonreg_trg_path, mode="r", encoding="utf-8"
            ) as nonreg_trg_file:
                i = 0
                # For Source, Target and FilePath
                for src_line, trg_line, nonreg_trg_line, files_line in zip(
                    src_file, trg_file, nonreg_trg_file, files_file
                ):
                    i += 1

                    # Strip away the "\n" at the end of the line
                    src_line, trg_line, nonreg_trg_line, files_line = (
                        src_line.strip(),
                        trg_line.strip(),
                        nonreg_trg_line.strip(),
                        files_line.strip(),
                    )

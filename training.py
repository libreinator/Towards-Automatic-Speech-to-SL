...
class TrainManager:
    def __init__(
        self, model: Model, disc: Classifier, config: dict, test=False
    ) -> None:
        train_config = config["training"]
        model_dir = train_config["model_dir"]
        # If model continue, continues model from the latest checkpoint
        model_continue = (
            True if not test else False
        )  # train_config.get("continue", True)
        ...
        self.disc = disc
        self.disc_opt = build_optimizer(
            config=config["training"]["disc"], parameters=self.disc.parameters()
        )
    ...
    def train_and_validate(self, train_data: Dataset, valid_data: Dataset) -> None:
        ...
        for epoch_no in range(self.epochs):
            ...
            for batch in iter(train_iter):
                ...
                batch_loss, noise, skel_out = self._train_batch(
                    batch, update=update, noise_rate=self.noise_rate
                )
                ...
                seq_valid = Variable(
                    Tensor(np.ones((batch.trg.shape[0], 1))), requires_grad=False
                )
                seq_fake_gt = Variable(
                    Tensor(np.zeros((batch.trg.shape[0], 1))), requires_grad=False
                )
                ...
                if self.future_prediction != 0:
                    ...
                    out = torch.cat(
                        (
                            skel_out[
                                :, :, : skel_out.shape[2] // (self.future_prediction)
                            ],
                            skel_out[:, :, -1:],
                        ),
                        dim=2,
                    ...
                    batch_out = torch.cat(
                        (
                            batch.trg[
                                :, :, : batch.trg.shape[2] // (self.future_prediction)
                            ],
                            batch.trg[:, :, -1:],
                        ),
                        dim=2,
                ...
                if self.future_prediction != 0:
                    batch_out = padding_tensor(
                        batch_out, out.shape[1] + self.future_prediction + 1
                    )
                    ...
                    if self.is_best(ckpt_score):
                        ...
                        display = list(
                            range(
                                ...
                                int(np.ceil(len(valid_hypotheses) / 10)),
                            )
                        )
    ...
    def produce_validation_video(
        ...
        for i in display:
            ...
            audio_path = os.path.join(
                "../../Data/", audio_name[:-1], "audio", variation + ".wav"
            )
    ...
    # Train the batch
    def _train_batch(
        self, batch: Batch, update: bool = True, noise_rate: int = 0
    ) -> Tensor:
        ...
        batch_loss, noise, skel_out = self.model.get_loss_for_batch(
            ...
            loss_function=self.loss,
            nonreg_loss_function=self.nonreg_loss_function,
            xentloss_weight=self.xentloss_weight,
            regloss_weight=self.regloss_weight,
...
def train(cfg_file: str) -> None:
    ...
    train_data, dev_data, test_data, src_vocab, trg_vocab, nonreg_trg_vocab = load_data(
        cfg=cfg
    )
    ...
    model, disc = build_model(
        cfg, src_vocab=src_vocab, trg_vocab=trg_vocab, nonreg_trg_vocab=nonreg_trg_vocab
    )
